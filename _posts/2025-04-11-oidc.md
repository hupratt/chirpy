...
I got to play around with other identity providers but I wanted something that would not cost a fortune, would respect an open standard and wouldn't bind me to any powerful tech mega corporation. Authentik seemed to fit the bill so let's start it up
...
root@authentik:$ ssh root@<server's ip> -i ~/.ssh/id_ecdsa
