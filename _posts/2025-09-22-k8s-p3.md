...
This is the third one in the series. We looked at how to install, configure kubernetes and deploy a web app with it. Now we'll take setup to another level in a 3 control plane, 3 workers on a 3 node cluster. 

### 1. Install the OS

#### A. Regular rocky linux 10 with ext4 partition

Installing the OS is done through hetzner's tool. Documentation on hetzner's program [can be found here](https://docs.hetzner.com/robot/dedicated-server/operating-systems/installimage/).
...
root@rescue:$ nano install-config.txt
...
#### B. Fedora server 42 with btrfs on root

On this setup I'll create an ext4 partition for the grub boot files and a btrfs raid 1 with the available space that is left. 
...
```bash
root@node0X:$ dnf upgrade -y && dnf install epel-release -y && dnf install fail2ban -y 
...
```bash
root@node01:$ dnf install python3 python3-pip git -y
...
[kube_control_plane]
node1 ansible_host=10.210.19.11 ip=10.210.19.11 etcd_member_name=etcd1
node2 ansible_host=10.210.19.12 ip=10.210.19.12 etcd_member_name=etcd2
node3 ansible_host=10.210.19.13 ip=10.210.19.13 etcd_member_name=etcd3

[etcd:children]
kube_control_plane

[kube_node]
node1 ansible_host=10.210.19.11
node2 ansible_host=10.210.19.12
node3 ansible_host=10.210.19.13
...
```
and copy over the public key to the 3 nodes ~/.ssh/authorized_keys
...
firewalld is enabled by default but was not running for some unknown reason 

```bash
root@node01:$ ssh 10.210.19.12 "systemctl start firewalld"
root@node01:$ ssh 10.210.19.12 "systemctl start firewalld"
root@node01:$ ssh 10.210.19.13 "systemctl start firewalld"
```
...
root@node02:$ systemctl restart firewalld && systemctl status firewalld
...
Allow NATing. Allow traffic to NodePort / ClusterIP

```bash
root@node0X:$ firewall-cmd --zone=public --add-port=53/udp --permanent
root@node0X:$ firewall-cmd --permanent --zone=public --set-target=ACCEPT
...
copy the certificate authority /etc/kubernetes/pki/ca.crt to your computer and install it in your keystore
