...
or configure your default  namespace by running
...
The client goes to the DNS server mongo-k8s.thekor.eu which points to 192.168.3.82 and 192.168.3.83. The worker node01 or worker node02 forwards it to the ingress controller. The request then gets forwarded to the web app's svc who forwards it to the pod

I've opted for dnsmasq as it allows you to specify 2 ip addresses for 1 A record as we don't have an haproxy load balancer just yet. 
